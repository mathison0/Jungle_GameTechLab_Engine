...
Lua는 `vcpkg.json` 기준으로 5.4.8 버전이 설치됩니다. `sol2`가 Lua 5.5를 지원하지 않으므로 Lua를 수동 설치하거나 `vcpkg install lua`만 따로 실행하지 않습니다.
...
Lua 사용 구성에서는 빌드 후 `lua.dll`이 출력 폴더로 자동 복사됩니다.
...
Lua 스크립팅은 `NipsEngine/Source/Engine/Scripting`과 `LuaScriptComponent`를 통해 연결됩니다. 프로젝트는 `WITH_LUA=1`, `SOL_ALL_SAFETIES_ON=1` 구성에서 Lua와 sol2를 사용하며, Lua 런타임은 vcpkg로 설치된 5.4.8 버전을 기준으로 합니다.
