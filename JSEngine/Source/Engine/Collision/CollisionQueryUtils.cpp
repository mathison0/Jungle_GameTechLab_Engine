...
static float ComputeAABBDepenetration(const FAABB& Bounds, const FVector& Point, FVector& OutNormal)
{
	float BestDistance = Point.X - Bounds.Min.X;
	FVector BestNormal = -FVector::UnitX();

	const float MaxXDistance = Bounds.Max.X - Point.X;
	if (MaxXDistance < BestDistance)
	{
		BestDistance = MaxXDistance;
		BestNormal = FVector::UnitX();
	}
	const float MinYDistance = Point.Y - Bounds.Min.Y;
	if (MinYDistance < BestDistance)
	{
		BestDistance = MinYDistance;
		BestNormal = -FVector::UnitY();
	}
	const float MaxYDistance = Bounds.Max.Y - Point.Y;
	if (MaxYDistance < BestDistance)
	{
		BestDistance = MaxYDistance;
		BestNormal = FVector::UnitY();
	}
	const float MinZDistance = Point.Z - Bounds.Min.Z;
	if (MinZDistance < BestDistance)
	{
		BestDistance = MinZDistance;
		BestNormal = -FVector::UnitZ();
	}
	const float MaxZDistance = Bounds.Max.Z - Point.Z;
	if (MaxZDistance < BestDistance)
	{
		BestDistance = MaxZDistance;
		BestNormal = FVector::UnitZ();
	}
...
bool FCollisionQueryUtils::RaycastInflatedAABB(const FAABB& Bounds, const FRay& Ray, float Inflation,
	bool bInitialOverlapAsHit, FHitResult& OutHitResult)
{
	...
	const FVector ExpandBy(Inflation, Inflation, Inflation);
	const FAABB InflatedBounds(Bounds.Min - ExpandBy, Bounds.Max + ExpandBy);
	...
	for (int32 AxisIndex = 0; AxisIndex < 3; ++AxisIndex)
	{
		const float Origin = Ray.Origin[AxisIndex];
		const float Direction = Ray.Direction[AxisIndex];
		const float MinValue = InflatedBounds.Min[AxisIndex];
		const float MaxValue = InflatedBounds.Max[AxisIndex];
		...
		FVector FarNormal = FVector::ZeroVector;
		NearNormal[AxisIndex] = -1.0f;
		FarNormal[AxisIndex] = 1.0f;
	...
	if (bStartsInside && bInitialOverlapAsHit)
	{
		...
		const float PushOutDistance = ComputeAABBDepenetration(InflatedBounds, Ray.Origin, HitNormal);
