...
void FShadowMapPass::SubmitDrawCommands(FRenderPipelineContext& Context)
{
    ...
    {
        const uint8 ItemIndex = static_cast<uint8>((Commands[CurrentIdx].SortKey >> 48) & 0xFF);
        const uint32 RangeStart = CurrentIdx;
        while (CurrentIdx < GlobalEnd && static_cast<uint8>((Commands[CurrentIdx].SortKey >> 48) & 0xFF) == ItemIndex)
        {
            ++CurrentIdx;
        }
