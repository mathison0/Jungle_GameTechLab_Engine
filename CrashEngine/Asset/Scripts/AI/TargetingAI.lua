...
function TargetingAI:Target(ComponentHandle, TargetTag, bZAxisOnly, DeltaTime)
...
function TargetingAI:TargetCoroutine(ComponentHandle, TargetTag, bZAxisOnly)
    while true do
        local deltaTime = Co.WaitNextFrame()
        self:Target(ComponentHandle, TargetTag, bZAxisOnly, deltaTime)
    end
end
