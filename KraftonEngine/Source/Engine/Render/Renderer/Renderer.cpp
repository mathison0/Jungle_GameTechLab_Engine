...
void FRenderer::BuildCommandForProxy(const FPrimitiveSceneProxy& Proxy, ERenderPass Pass)
{
	...
	FShader* ResolvedShader = ResolvePipelineShader(ViewModePassRegistry, ActiveViewMode, Pass, Proxy.Shader);
...
{
	if (!bHasActiveViewModePassConfig || !DecalProxy.DiffuseSRV)
	{
		return;
	}
	...
	FShader* DecalShader = ResolvePipelineShader(ViewModePassRegistry, ActiveViewMode, ERenderPass::Decal, DecalProxy.Shader);
	if (!DecalShader)
	{
		return;
	}
...
void FRenderer::BuildDynamicCommands(const FFrameContext& Frame, const FScene* Scene)
{
	...
	if (bHasActiveViewModePassConfig && ActiveViewSurfaceSet)
	{
		BuildUberLitPassCommand(DrawCommandList, PassRenderStates, ViewModePassRegistry, ActiveViewMode);
	}
}

// ============================================================
// PreparePipelineExecution — 정렬 + 제출 준비
...
void FRenderer::PreparePipelineExecution(const FFrameContext& Frame)
{
	...
	PipelinePrePassEvents.clear();
	BuildDefaultPassEvents(PipelinePrePassEvents, Context, Frame, PipelineStateCache, ViewModePassRegistry, ActiveViewMode, ActiveViewSurfaceSet);
	bPipelineExecutionPrepared = true;
}

void FRenderer::SubmitRenderPass(ERenderPass CurPass)
{
	...
	ID3D11DeviceContext* Context = Device.GetDeviceContext();
	for (auto& PrePassEvent : PipelinePrePassEvents)
	{
	...
	DrawCommandList.SubmitRange(Start, End, Device, Context, PipelineStateCache);
}


void FRenderer::SubmitRenderPassByUserBits(ERenderPass CurPass, uint16 UserBits)
{
	if (!bPipelineExecutionPrepared)
	{
		return;
	...
	const TArray<FDrawCommand>& Commands = DrawCommandList.GetCommands();
	if (Commands.empty())
	{
		return;
	}
	...
	for (uint32 i = Start; i < End; ++i)
	{
		...
		if ((Cmd.SortKey & 0xFFFu) != UserBits)
		{
			continue;
		}
		DrawCommandList.SubmitRange(i, i + 1, Device, Context, PipelineStateCache);
	}
}
