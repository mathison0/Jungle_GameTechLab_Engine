...
namespace
{
	...
	const char* GetViewModeName(EViewMode Mode)
	{
        ...
        case EViewMode::DepthScene: return "Depth Scene";
		case EViewMode::Fog: return "Fog";
		case EViewMode::Lit_Gouraud:return "Lit_Gouraud";
...
void FEditorMainPanel::RenderViewportHostWindow()
{
	...
	if (ContentSize.x > 1.0f && ContentSize.y > 1.0f)
	{
		...
		if (ID3D11ShaderResourceView* SceneColorSRV = EditorEngine->GetRenderer().GetFD3DDevice().GetViewportDisplaySRV())
		{
...
void FEditorMainPanel::RenderViewportMenuBarForIndex(int32 Index)
{
	...
	if (ImGui::BeginMenu("View"))
	{
		...
		static constexpr const char* Labels[] = { "Lit", "Unlit", "Wireframe", "DepthScene", "Fog", "Lit_Gouraud", "Lit_Lambert", "Lit_Phong"};
	...
	{
		FViewportPostProcessSettings& PostProcessSettings = State.PostProcessSettings;
		FFXAASettings& LocalFXAASettings = PostProcessSettings.LocalFXAASettings;
		const FFXAASettings& GlobalFXAASettings = EditorEngine->GetSettings().DefaultFXAASettings;

		ImGui::MenuItem("Enabled", nullptr, &PostProcessSettings.bFXAAEnabled);
		ImGui::Separator();
		ImGui::MenuItem("Use Global Params", nullptr, &PostProcessSettings.bUseGlobalFXAASettings);

		if (PostProcessSettings.bUseGlobalFXAASettings)
		{
			ImGui::TextDisabled("Subpix: %.2f", GlobalFXAASettings.Subpix);
			ImGui::TextDisabled("Edge Threshold: %.3f", GlobalFXAASettings.EdgeThreshold);
			ImGui::TextDisabled("Edge Threshold Min: %.4f", GlobalFXAASettings.EdgeThresholdMin);

			if (ImGui::MenuItem("Copy Global To Local"))
			{
				LocalFXAASettings = GlobalFXAASettings;
			}
		}
		else
		{
			if (ImGui::MenuItem("Reset Local To Global"))
			{
				LocalFXAASettings = GlobalFXAASettings;
			}

			ImGui::SetNextItemWidth(160.0f);
			ImGui::SliderFloat("Subpix", &LocalFXAASettings.Subpix, 0.0f, 1.0f, "%.2f");

			ImGui::SetNextItemWidth(160.0f);
			ImGui::SliderFloat("Edge Threshold", &LocalFXAASettings.EdgeThreshold, 0.0312f, 0.333f, "%.3f");

			ImGui::SetNextItemWidth(160.0f);
			ImGui::SliderFloat("Edge Threshold Min", &LocalFXAASettings.EdgeThresholdMin, 0.0f, 0.0833f, "%.4f");
		}
