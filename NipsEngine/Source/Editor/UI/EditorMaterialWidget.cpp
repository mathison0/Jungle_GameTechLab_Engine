...
void FEditorMaterialWidget::Render(float DeltaTime)
{
	...
	USceneComponent* TargetComp = GetSelectedTargetComponent();
    if (!TargetComp)
    {
        ImGui::TextDisabled("Select a StaticMesh or Decal actor to edit materials.");
        ImGui::End();
        return;
    }

    UStaticMeshComponent* MeshComp = dynamic_cast<UStaticMeshComponent*>(TargetComp);
    UDecalComponent*      DecalComp = dynamic_cast<UDecalComponent*>(TargetComp);

	bool bChanged = false;
    if (MeshComp && (MeshComp != LastMeshComp || MeshComp->GetStaticMesh() != LastMeshAsset))
    {
        LastMeshComp = MeshComp;
        LastMeshAsset = MeshComp->GetStaticMesh();
        LastDecalComp = nullptr;
        bChanged = true;
    }
    else if (DecalComp && DecalComp != LastDecalComp)
    {
        LastDecalComp = DecalComp;
        LastMeshComp = nullptr;
        LastMeshAsset = nullptr;
        bChanged = true;
    }
    ...
    RenderSectionList(MeshComp, DecalComp); // 파라미터 변경됨
    ImGui::EndChild();

    ImGui::SameLine();

    ImGui::BeginChild("##MaterialDetails", ImVec2(0, 0), true);
    RenderMaterialDetails(MeshComp, DecalComp); // 파라미터 변경됨
    ImGui::EndChild();
...
// 왼쪽: 섹션 목록
// -----------------------------------------------------------------------
void FEditorMaterialWidget::RenderSectionList(UStaticMeshComponent* MeshComp, UDecalComponent* DecalComp)
{
    if (MeshComp)
    {
    ...
    {
        ImGui::Text("Sections (1)");
        ImGui::Separator();

        if (SelectedSectionIndex < 0)
        {
            SelectedSectionIndex = 0;
            SelectedMaterialPtr = const_cast<FMaterial*>(DecalComp->GetDecalMaterial());
        }

        bool bMissing = (SelectedMaterialPtr == nullptr);
        char Label[128];
        snprintf(Label, sizeof(Label), "[0] Decal Mat%s", bMissing ? " (!)" : "");

        if (bMissing)
            ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.0f, 0.4f, 0.4f, 1.0f));

        if (ImGui::Selectable(Label, (SelectedSectionIndex == 0), 0, ImVec2(0, 20)))
        {
            SelectedSectionIndex = 0;
            SelectedMaterialPtr = const_cast<FMaterial*>(DecalComp->GetDecalMaterial());
        }
...
// 오른쪽: 머테리얼 상세 (복사본 편집)
// -----------------------------------------------------------------------
void FEditorMaterialWidget::RenderMaterialDetails(UStaticMeshComponent* MeshComp, UDecalComponent* DecalComp)
{
    if (SelectedSectionIndex < 0)
    {
	...
	if (MeshComp)
	{
        ...
        {
            ImGui::Text("Section [0]  |  Slot: Decal Mat");
        }
    ...
    if (ImGui::BeginCombo("##MaterialCombo", PreviewLabel))
    {
        for (int32 i = 0; i < static_cast<int32>(MatNames.size()); ++i)
        {
		    ...
		    if (MeshComp)
		    {
                    ...
                    {
                        DecalComp->SetDecalMaterial(NewMat);
                    }
...
// -----------------------------------------------------------------------
USceneComponent* FEditorMaterialWidget::GetSelectedTargetComponent() const
{
    ...
    {
        // 컴포넌트가 존재하고, 타입이 일치하면 바로 반환
        if (Comp && (Comp->IsA<UStaticMeshComponent>() || Comp->IsA<UDecalComponent>()))
        {
            return dynamic_cast<USceneComponent*>(Comp);
        }
    }
