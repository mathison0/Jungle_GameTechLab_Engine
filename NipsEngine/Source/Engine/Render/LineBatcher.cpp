...
void FLineBatcher::AddOBB(const FMatrix& Transform, const FColor& InColor)
{
	const FVector4 Color = InColor.ToVector4();
	const uint32 BaseVertex = static_cast<uint32>(IndexedVertices.size());
	...
	for (const FVector& LocalPos : LocalCorners)
	{
		IndexedVertices.emplace_back(Transform.TransformPosition(LocalPos), Color);
	}
