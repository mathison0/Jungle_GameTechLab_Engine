...
public:
    ...
    virtual ADefaultPawn* SpawnDefaultPawnFallback();
...
protected:
	...
	// 기본 PlayerController는 GameClient/PIE 시작 시 플레이어 Pawn과 Camera를 보장하고,
	// GameInputBridge/GameEngine에서 만든 gameplay action snapshot을 Pawn 쪽으로 전달합니다.
	virtual void ApplyDefaultPawnFallbackTransform(ADefaultPawn* DefaultPawn);

	void DispatchInputActionsToPawn();
	void UpdateDefaultPawnFromInput(float DeltaTime);
    ...
    bool IsPossessingDefaultPawn() const;
