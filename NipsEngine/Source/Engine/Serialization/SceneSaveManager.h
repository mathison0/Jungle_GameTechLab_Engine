...
private:
	...
	static json::JSON SerializeActorComponent(UActorComponent* Comp);
	...
	// ---- Deserialization ----
	static void DeserializeActorsToWorld(json::JSON& PrimitivesNode, UWorld* World);
	static void DeserializePrimitivesToWorld(json::JSON& PrimitivesNode, UWorld* World);
	static USceneComponent* DeserializeSceneComponentTree(json::JSON& Node, AActor* Owner, USceneComponent* ExistingComp = nullptr);
