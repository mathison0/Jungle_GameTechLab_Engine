	...
	Core.GetRenderer()->SetGUIUpdateCallback([&]()
	{
		...
		if (GSelectedActor)
		{
			...
			if (Root)
			{
				...
				const FVector EulerDegrees = Rotation.Euler();
				...
				float Rot[3] = { EulerDegrees.X, EulerDegrees.Y, EulerDegrees.Z };
				float Scl[3] = {
				...
				{
					Transform.SetTranslation({ Loc[0], Loc[1], Loc[2] });
					Root->SetRelativeTransform(Transform);
				}
				if (ImGui::DragFloat3("Rotation", Rot, 0.1f))
				{
					Transform.SetRotation(FRotator::MakeFromEuler({ Rot[0], Rot[1], Rot[2] }));
					Root->SetRelativeTransform(Transform);
				}
				if (ImGui::DragFloat3("Scale", Scl, 0.1f))
				{
					Transform.SetScale3D({ Scl[0], Scl[1], Scl[2] });
					Root->SetRelativeTransform(Transform);
				}
			}
		}
