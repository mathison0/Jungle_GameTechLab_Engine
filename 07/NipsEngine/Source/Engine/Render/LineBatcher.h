...
class FLineBatcher
{
  public:
    ...
    void AddCircleOnPlane(const FVector& Center, const FVector& PlaneNormal, float Radius, int32 SegmentCount,
                          const FVector4& Color);
    // Sphere
    void AddSphere(const FVector& Center, float Radius, int32 SegmentCount, const FVector4& Color);
    ...
    void AddWorldHelpers(const FShowFlags& ShowFlags, float GridSpacing, int32 GridHalfLineCount,
                         const FVector& CameraPosition, const FVector& CameraForward, bool bOrthographic = false);
