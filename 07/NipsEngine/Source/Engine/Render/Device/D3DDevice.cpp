...
void FD3DDevice::BeginFrame()
{
    ...
    if (ViewportSceneColorRTV && ViewportSelectionMaskRTV && ViewportDepthStencilView)
    {
        ...
        if (ViewportPostProcessSceneColorRTV)
        {
            DeviceContext->ClearRenderTargetView(ViewportPostProcessSceneColorRTV.Get(), ClearColor);
        }
        DeviceContext->ClearRenderTargetView(ViewportSelectionMaskRTV.Get(), ClearMask);
        DeviceContext->ClearDepthStencilView(ViewportDepthStencilView.Get(), D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL,
                                             1.0f, 0);
    }
...
void FD3DDevice::EnsureViewportRenderTargets(int Width, int Height)
{
    ...
    if (ViewportRenderTargetWidth == SafeWidth && ViewportRenderTargetHeight == SafeHeight && ViewportSceneColorRTV &&
        ViewportPostProcessSceneColorRTV && ViewportSelectionMaskRTV && ViewportDepthStencilView)
    {
        return;
    }
...
void FD3DDevice::SetSubViewport(int32 X, int32 Y, int32 Width, int32 Height)
{
    ...
    CurrentSubViewport = vp;
}

ID3D11Device* FD3DDevice::GetDevice() const { return Device.Get(); }

ID3D11DeviceContext* FD3DDevice::GetDeviceContext() const { return DeviceContext.Get(); }
...
FRenderTargetSet FD3DDevice::GetBackBufferRenderTargets() const
{
    ...
    Targets.DepthStencilSRV = DepthStencilSRV.Get();
...
FRenderTargetSet FD3DDevice::GetViewportRenderTargets() const
{
    ...
    Targets.PostProcessSceneColorRTV = ViewportPostProcessSceneColorRTV.Get();
    Targets.PostProcessSceneColorSRV = ViewportPostProcessSceneColorSRV.Get();
...
{
    if (CurrentDepthStencilState == InState)
        return;
    ...
    case EDepthStencilState::DepthNone:
        DeviceContext->OMSetDepthStencilState(DepthStencilStateDepthNone.Get(), 1);
        break;
...
{
    if (CurrentRasterizerState == InState)
        return;
...
void FD3DDevice::SetClearColor(float R, float G, float B)
{
    ClearColor[0] = R;
    ClearColor[1] = G;
    ClearColor[2] = B;
    // ClearColor[3] (Alpha)는 항상 1.0f 유지
}
...
void FD3DDevice::CreateDeviceAndSwapChain(HWND InHWindow)
{
    ...
    TComPtr<IDXGIFactory5> Factory5;
    {
        ...
        {
            if (SUCCEEDED(Factory1->QueryInterface(__uuidof(IDXGIFactory5),
                                                   reinterpret_cast<void**>(Factory5.GetAddressOf()))))
            {
                Factory5->CheckFeatureSupport(DXGI_FEATURE_PRESENT_ALLOW_TEARING, &bTearingSupported,
    ...
    D3D11CreateDeviceAndSwapChain(nullptr, D3D_DRIVER_TYPE_HARDWARE, nullptr, CreateDeviceFlags, featureLevels,
                                  ARRAYSIZE(featureLevels), D3D11_SDK_VERSION, &swapChainDesc, SwapChain.GetAddressOf(),
    ...
    if (Device)
    {
        ...
        {
            OutputDebugStringA(
                "[D3D11] Debug layer is not available. Install Graphics Tools or ensure debug runtime is present.\n");
        }
    }
#endif
}
...
{
    SwapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), reinterpret_cast<void**>(FrameBuffer.ReleaseAndGetAddressOf()));

    CD3D11_RENDER_TARGET_VIEW_DESC frameBufferRTVDesc = {};
    frameBufferRTVDesc.Format = DXGI_FORMAT_B8G8R8A8_UNORM;
    frameBufferRTVDesc.ViewDimension = D3D11_RTV_DIMENSION_TEXTURE2D;

    Device->CreateRenderTargetView(FrameBuffer.Get(), &frameBufferRTVDesc, FrameBufferRTV.ReleaseAndGetAddressOf());
    ...
    Device->CreateTexture2D(&selectionMaskDesc, nullptr, SelectionMaskBuffer.ReleaseAndGetAddressOf());
...
void FD3DDevice::CreateViewportRenderTargets(uint32 Width, uint32 Height)
{
    ...
    Device->CreateTexture2D(&sceneColorDesc, nullptr, ViewportSceneColorTexture.ReleaseAndGetAddressOf());
    ...
    Device->CreateTexture2D(&sceneColorDesc, nullptr, ViewportPostProcessSceneColorTexture.ReleaseAndGetAddressOf());
    Device->CreateRenderTargetView(ViewportPostProcessSceneColorTexture.Get(), &sceneColorRTVDesc,
                                   ViewportPostProcessSceneColorRTV.ReleaseAndGetAddressOf());
    Device->CreateShaderResourceView(ViewportPostProcessSceneColorTexture.Get(), &sceneColorSRVDesc,
                                     ViewportPostProcessSceneColorSRV.ReleaseAndGetAddressOf());
    ...
    selectionMaskDesc.Usage = D3D11_USAGE_DEFAULT;
    selectionMaskDesc.BindFlags = D3D11_BIND_RENDER_TARGET | D3D11_BIND_SHADER_RESOURCE;
    Device->CreateTexture2D(&selectionMaskDesc, nullptr, ViewportSelectionMaskTexture.ReleaseAndGetAddressOf());
...
void FD3DDevice::ReleaseViewportRenderTargets()
{
    ...
    ViewportPostProcessSceneColorSRV.Reset();
    ViewportPostProcessSceneColorRTV.Reset();
    ViewportPostProcessSceneColorTexture.Reset();
...
void FD3DDevice::CreateRasterizerState()
{
    ...
    Device->CreateRasterizerState(&rasterizerDesc, RasterizerStateBackCull.ReleaseAndGetAddressOf());

    D3D11_RASTERIZER_DESC frontCullDesc = {};
    frontCullDesc.FillMode = D3D11_FILL_SOLID;
    frontCullDesc.CullMode = D3D11_CULL_FRONT;

    Device->CreateRasterizerState(&frontCullDesc, RasterizerStateFrontCull.ReleaseAndGetAddressOf());

    D3D11_RASTERIZER_DESC noCullDesc = {};
    noCullDesc.FillMode = D3D11_FILL_SOLID;
    noCullDesc.CullMode = D3D11_CULL_NONE;

    Device->CreateRasterizerState(&noCullDesc, RasterizerStateNoCull.ReleaseAndGetAddressOf());

    D3D11_RASTERIZER_DESC wireFrameDesc = {};
    wireFrameDesc.FillMode = D3D11_FILL_WIREFRAME;
    wireFrameDesc.CullMode = D3D11_CULL_NONE;

    Device->CreateRasterizerState(&wireFrameDesc, RasterizerStateWireFrame.ReleaseAndGetAddressOf());
...
void FD3DDevice::CreateDepthStencilBuffer()
{
    ...
    Device->CreateDepthStencilView(DepthStencilBuffer.Get(), &dsvDesc, DepthStencilView.ReleaseAndGetAddressOf());

    D3D11_SHADER_RESOURCE_VIEW_DESC srvDesc = {};
    srvDesc.Format = DXGI_FORMAT_R24_UNORM_X8_TYPELESS;
    srvDesc.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE2D;
    srvDesc.Texture2D.MipLevels = 1;
    Device->CreateShaderResourceView(DepthStencilBuffer.Get(), &srvDesc, DepthStencilSRV.ReleaseAndGetAddressOf());
    ...
    Device->CreateDepthStencilState(&depthStencilStateDefaultDesc, DepthStencilStateDefault.ReleaseAndGetAddressOf());
    ...
    D3D11_DEPTH_STENCIL_DESC depthStencilDepthNoneDesc = {};
    ZeroMemory(&depthStencilDepthNoneDesc, sizeof(depthStencilDepthNoneDesc));
    depthStencilDepthNoneDesc.DepthEnable = FALSE;
    depthStencilDepthNoneDesc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ZERO;
    depthStencilDepthNoneDesc.DepthFunc = D3D11_COMPARISON_ALWAYS;
    depthStencilDepthNoneDesc.StencilEnable = FALSE;

    Device->CreateDepthStencilState(&depthStencilDepthNoneDesc, DepthStencilStateDepthNone.ReleaseAndGetAddressOf());
    ...
    { // Gizmo split by selected-object stencil mask (ref=1)
        ...
        Device->CreateDepthStencilState(&gizmoInsideDesc, DepthStencilStateGizmoInside.ReleaseAndGetAddressOf());
        ...
        Device->CreateDepthStencilState(&gizmoOutsideDesc, DepthStencilStateGizmoOutside.ReleaseAndGetAddressOf());
    }
    ...
    Device->CreateDepthStencilState(&maskDesc, DepthStencilStateStencilMaskEqual.ReleaseAndGetAddressOf());
}
...
void FD3DDevice::ReleaseDepthStencilBuffer()
{
    ...
    DepthStencilStateDepthNone.Reset();
