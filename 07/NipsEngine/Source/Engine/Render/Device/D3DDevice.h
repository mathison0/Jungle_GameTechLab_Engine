...
struct FRenderTargetSet
{
	...
	ID3D11RenderTargetView* PostProcessSceneColorRTV = nullptr;
	ID3D11ShaderResourceView* PostProcessSceneColorSRV = nullptr;
...
private:
	...
	TComPtr<ID3D11Texture2D> ViewportPostProcessSceneColorTexture;
	TComPtr<ID3D11RenderTargetView> ViewportPostProcessSceneColorRTV;
	TComPtr<ID3D11ShaderResourceView> ViewportPostProcessSceneColorSRV;
    ...
    TComPtr<ID3D11DepthStencilState> DepthStencilStateDepthNone;
	...
	// 다중 뷰포트 정보
	D3D11_VIEWPORT CurrentSubViewport = { };
...
public:
	...
	ID3D11ShaderResourceView* GetViewportDisplaySRV() const
	{
		return ViewportPostProcessSceneColorSRV.Get() ? ViewportPostProcessSceneColorSRV.Get() : ViewportSceneColorSRV.Get();
	}
	float GetViewportWidth() const { return ViewportInfo.Width; }
	float GetViewportHeight() const { return ViewportInfo.Height; }
	const D3D11_VIEWPORT& GetSubViewportInfo() const { return CurrentSubViewport; }
