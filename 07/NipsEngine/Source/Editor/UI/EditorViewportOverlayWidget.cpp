...
void FEditorViewportOverlayWidget::RenderViewportSettings(float DeltaTime)
{
    ...
    const float ItemWidth = ImGui::GetContentRegionAvail().x * 0.5f;
    auto        SetControlWidth = [ItemWidth]() { ImGui::SetNextItemWidth(ItemWidth); };
    ...
    if (BeginSettingsSection("Show Flags", true))
    {
        ...
        ImGui::Checkbox("Show Bounding Volume", &Settings.ShowFlags.bBoundingVolume);
        if (Settings.ShowFlags.bBoundingVolume)
        {
            ImGui::Indent();
            ImGui::Checkbox("Show BVH Bounding Volume", &Settings.ShowFlags.bBVHBoundingVolume);
            ImGui::Unindent();
        }
    }
    ...
    {
        SetControlWidth();
        ImGui::SliderFloat("Subpix", &Settings.DefaultFXAASettings.Subpix, 0.0f, 1.0f, "%.2f");

        SetControlWidth();
        ImGui::SliderFloat("Edge Threshold", &Settings.DefaultFXAASettings.EdgeThreshold, 0.0312f, 0.333f, "%.3f");

        SetControlWidth();
        ImGui::SliderFloat("Edge Threshold Min", &Settings.DefaultFXAASettings.EdgeThresholdMin, 0.0f, 0.0833f, "%.4f");
    }

    if (BeginSettingsSection("Grid", false))
    {
        SetControlWidth();
        ...
        SetControlWidth();
        ImGui::SliderFloat("Line Thickness", &Settings.GridLineThickness, 0.1f, 3.0f, "%.2f");

        SetControlWidth();
        ImGui::SliderFloat("Major Line Thickness", &Settings.GridMajorLineThickness, 0.1f, 4.0f, "%.2f");

        SetControlWidth();
        ImGui::SliderFloat("Major Line Interval", &Settings.GridMajorLineInterval, 2.0f, 20.0f, "%.0f");

        SetControlWidth();
        ImGui::SliderFloat("Minor Intensity", &Settings.GridMinorIntensity, 0.0f, 1.0f, "%.2f");

        SetControlWidth();
        ImGui::SliderFloat("Major Intensity", &Settings.GridMajorIntensity, 0.0f, 1.0f, "%.2f");

        SetControlWidth();
        ImGui::SliderFloat("Range Scale", &Settings.GridRangeScale, 0.25f, 4.0f, "%.2f");

        SetControlWidth();
        ImGui::SliderFloat("Fade Distance Scale", &Settings.GridMaxDistanceScale, 0.25f, 4.0f, "%.2f");
    }

    if (BeginSettingsSection("Axis", false))
    {
        SetControlWidth();
        ImGui::SliderFloat("Axis Thickness", &Settings.AxisThickness, 0.1f, 6.0f, "%.2f");

        SetControlWidth();
        ImGui::SliderFloat("Axis Length Scale", &Settings.AxisLengthScale, 0.25f, 4.0f, "%.2f");
    }

    if (BeginSettingsSection("BVH", false))
    {
...
void FEditorViewportOverlayWidget::RenderDebugStats(float DeltaTime)
{
    ...
    for (int32 i = 0; i < FViewportLayout::MaxViewports; ++i)
    {
        ...
        // 툴바 바로 아래 좌측에 고정
        ImGui::SetNextWindowPos(ImVec2(static_cast<float>(VS.Rect.X) + 8.f, static_cast<float>(VS.Rect.Y) + 32.f),
                                ImGuiCond_Always);
        ...
        if (ImGui::Begin(WinId, nullptr, kFlags))
        {
            ...
            if (CullingStats != nullptr)
            {
                const int32 CulledPrimitiveCount = std::max(
                    0, CullingStats->TotalVisiblePrimitiveCount -
                ...
                ImGui::TextColored(ImVec4(0.25f, 0.9f, 1.0f, 1.0f), "- Total Visible: %d",
                                   CullingStats->TotalVisiblePrimitiveCount);
                ImGui::TextColored(ImVec4(0.25f, 0.9f, 1.0f, 1.0f), "- BVH Passed: %d",
                                   CullingStats->BVHPassedPrimitiveCount);
                ImGui::TextColored(ImVec4(0.25f, 0.9f, 1.0f, 1.0f), "- Fallback Passed: %d",
            ...
            if (DecalStats != nullptr)
            {
                ...
                ImGui::TextColored(ImVec4(0.25f, 0.9f, 1.0f, 1.0f), "Decal");
                ImGui::TextColored(ImVec4(0.25f, 0.9f, 1.0f, 1.0f), "- Total Decal Count: %d",
                                   DecalStats->TotalDecalCount);
                ImGui::TextColored(ImVec4(0.25f, 0.9f, 1.0f, 1.0f), "- Total Decal Visible: %d",
                                   DecalStats->TotalVisibleDecalCount);
                /*ImGui::TextColored(ImVec4(0.25f, 0.9f, 1.0f, 1.0f), "- BVH Passed: %d",
                                   DecalStats->FallbackPassedDecalCount);*/
            }
            ...
            if (VS.bShowStatMemory)
            {
                ...
                for (TObjectIterator<UStaticMesh> It; It; ++It)
                {
                    ...
                    {
                        MeshMemoryBytes += sizeof(UStaticMesh) + Mesh->GetVertices().size() * sizeof(FNormalVertex) +
                ...
                ImGui::TextColored(ImVec4(1.f, 1.f, 0.f, 1.f), "- Total Allocated Counts: %d",
                                   EngineStatics::GetTotalAllocationCount());
                ImGui::TextColored(ImVec4(1.f, 1.f, 0.f, 1.f), "- Total Allocated Bytes: %.2f KB",
...
void FEditorViewportOverlayWidget::RenderSplitterBar()
{
    ...
    if (!ViewportLayout.IsSingleViewportMode())
    {
        ...
        SSplitter* Splitters[] = {ViewportLayout.GetRootSplitterV(), ViewportLayout.GetTopSplitterH(),
        ...
        for (SSplitter* S : Splitters)
        {
            ...
            const bool       bSplitterHover = !bIsDragging && ((S == Hovered || S == Captured) ||
                                                         (Linked && (Linked == Hovered || Linked == Captured)));

            ImU32 Color = BarColor;
            if (bCrossHovered)
                Color = CrossHoverColor;
            else if (bSplitterHover)
                Color = HoverColor;

            DrawList->AddRectFilled(ImVec2(Bar.X, Bar.Y), ImVec2(Bar.X + Bar.Width, Bar.Y + Bar.Height), Color);
        }
        ...
        if (Cross)
        {
            ...
            DrawList->AddRectFilled(ImVec2(CR.X, CR.Y), ImVec2(CR.X + CR.Width, CR.Y + CR.Height),
