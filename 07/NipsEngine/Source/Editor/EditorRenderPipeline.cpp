...
void FEditorRenderPipeline::RenderViewport(FRenderer& Renderer, int32 ViewportIndex)
{
    ...
    const FViewportPostProcessSettings& PostProcessSettings = VC.GetViewportState()->PostProcessSettings;
    ...
    GridConstants.GridSpacing = Settings.GridSpacing;
    GridConstants.GridHalfLineCount = Settings.GridHalfLineCount;
    GridConstants.LineThickness = Settings.GridLineThickness;
    GridConstants.MajorLineThickness = Settings.GridMajorLineThickness;
    GridConstants.MajorLineInterval = Settings.GridMajorLineInterval;
    GridConstants.MinorIntensity = Settings.GridMinorIntensity;
    GridConstants.MajorIntensity = Settings.GridMajorIntensity;
    GridConstants.RangeScale = Settings.GridRangeScale;
    GridConstants.MaxDistanceScale = Settings.GridMaxDistanceScale;
    GridConstants.AxisThickness = Settings.AxisThickness;
    GridConstants.AxisLengthScale = Settings.AxisLengthScale;
    Collector.CollectGrid(GridConstants, Bus);
    ...
    const FFXAASettings* ActiveFXAASettings = nullptr;
    FFXAASettings ResolvedFXAASettings = Settings.DefaultFXAASettings;
    if (PostProcessSettings.bFXAAEnabled && ViewMode != EViewMode::DepthScene)
    {
        if (!PostProcessSettings.bUseGlobalFXAASettings)
        {
            ResolvedFXAASettings = PostProcessSettings.LocalFXAASettings;
        }
        ActiveFXAASettings = &ResolvedFXAASettings;
    }

    Renderer.Render(Bus, ActiveFXAASettings);
}
