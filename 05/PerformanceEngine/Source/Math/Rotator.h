...
struct FRotator
{
	...
	// Axis-order input: X=Roll, Y=Pitch, Z=Yaw.
	constexpr FRotator(float InXDegrees, float InYDegrees, float InZDegrees) noexcept
		: Roll(InXDegrees), Pitch(InYDegrees), Yaw(InZDegrees)
	{
	}
	...
	// Euler vector order is {X(Roll), Y(Pitch), Z(Yaw)}.
	...
	// Returns Euler vector in {X(Roll), Y(Pitch), Z(Yaw)} order.
	...
	// Adds axis-order deltas {X(Roll), Y(Pitch), Z(Yaw)}.
	FRotator Add(float DeltaXDegrees, float DeltaYDegrees, float DeltaZDegrees) noexcept;
